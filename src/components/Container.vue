...
export default {
    ...
    methods: {
      async getData() {
        try {
          ...
          this.TodoList = r.data.listItems.items;
        }
        catch(error){
      ...
      async createItem() {
        ...
        const item = {name }
        try {
          const r = await API.graphql(graphqlOperation(createItem, {input: item}))
          console.log('Item created...')
          this.TodoLists = [...this.TodoList, r.data.createItem];
        }
        catch (error) {
      ...
      async deleteItem(id) {
        if(id){
          try {
            await API.graphql(graphqlOperation(deleteItem, {input: {id: id, "_version": "1"} }))
    ...
    API.graphql(graphqlOperation(onCreateItem)).subscribe(
      (sourceData) => {
        ...
        if (newItem) {
          if (this.TodoLists.some(r => r.id = newItem)) return;
          this.TodoLists = [newItem, ...this.TodoLists];
        }
      }
    )

    API.graphql(graphqlOperation(onDeleteItem)).subscribe(
      (sourceData) => {
        const removedItem = sourceData.value.data.onDeleteItem;
        if (removedItem){
          this.TodoLists = this.TodoLists.filter(
            (r) => r.id != removedItem.id
          )
        }
      }
    )
